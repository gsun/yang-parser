...
class AstIfFeatureVisitor extends AstVisitor {  
    ...
    function if_feature_stmt(stmt:Stmt, context:Dynamic) {
        ...
        if (local) {
            ...
            assertTrue(stmt.ref != null, 'uses_stmt ${stmt.arg} local-feature-reference-error');
        } else {
            var prefixName:Array<String> = stmt.arg.split(':');
            ...
            assertTrue(stmt.ref != null, 'if_feature_stmt ${stmt.arg} global-feature-reference-error');         
        }
    }   
}