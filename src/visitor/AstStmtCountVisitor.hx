...
class AstStmtCountVisitor extends AstVisitor {
	...
	function input_stmt(stmt:Stmt) {
		var num_of_data_def_stmt = stmt.subs.container_stmt.length + stmt.subs.leaf_stmt.length + stmt.subs.leaf_list_stmt.length
			+ stmt.subs.list_stmt.length + stmt.subs.choice_stmt.length + stmt.subs.anyxml_stmt.length + stmt.subs.uses_stmt.length;
