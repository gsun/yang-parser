...
class AstDefaultValidateVisitor extends AstVisitor {
	...
	function type_stmt(stmt:TypeStmt) {
		...
		if (default_stmt != null) {
			switch stmt.arg {
				...
				case "int64" | "uint64":
					validateInt64Range(stmt, default_stmt.arg);
				case "decimal64":
					validateDecimalRange(stmt, default_stmt.arg);
				case "string":
					validateString(stmt, default_stmt.arg);
				case "boolean":
				...
				case "binary":
					validateBinary(stmt, default_stmt.arg);
				case "leafref":
					if (isUnionBranch)
				...
				case "instance-identifier":
					if (isUnionBranch)
						validUnionBranch = false;
				case "empty":
					assertTrue(false, 'default_stmt ${default_stmt.arg} empty-error');
				case "union":
					validateUnion(stmt, default_stmt);
				default:
					validateUserType(stmt, default_stmt);
			}
	...
	function validateIntRange(stmt:Stmt, value:String) {
		...
		if (rangeArray.length > 0) {
			var e = rangeArray.find(function(e) {
				return (e.min == "min" || Std.parseInt(e.min) <= intValue) && (e.max == "max" || Std.parseInt(e.max) >= intValue);
			});
	...
	function validateInt64Range(stmt:Stmt, value:String) {
		...
		if (rangeArray.length > 0) {
			var e = rangeArray.find(function(e) {
				return (e.min == "min" || Std.parseFloat(e.min) <= intValue) && (e.max == "max" || Std.parseFloat(e.max) >= intValue);
			});
			assertTrue(e != null, 'value ${value} range-error');
		}
	}
	...
	function validateEnum(stmt:Stmt, value:String) {
		...
		var e = enum_stmts.find(function(e) {
			return e.arg == value;
		});
	...
	function validatebit(stmt:Stmt, value:String) {
		...
		for (v in valueList) {
			var e = bit_stmts.find(function(e) {
				return e.arg == v;
			});
	...
	function validateString(stmt:Stmt, value:String) {
		...
		if (rangeArray.length > 0) {
			var e = rangeArray.find(function(e) {
				return (e.min == "min" || Std.parseInt(e.min) <= value.length) && (e.max == "max" || Std.parseInt(e.max) >= value.length);
			});
